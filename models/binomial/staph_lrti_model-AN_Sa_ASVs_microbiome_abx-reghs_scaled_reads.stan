...
model {
  ...
  alpha ~ normal(0, 1);
  for ( i in 1:n ) {
        p[i] = beta[seqvar_id[i]] * read_count[i] + b_total * specimen_read_total[i] + b_asv_241b * asv_241b2b5f1ea0f1065ca7027534068198[i] + b_asv_adcd * asv_adcde8c78396022660fd6204471c65c8[i] + b_asv_f011 * asv_f011d78941f0c011aca75252ef604d17[i] + b_abx_vanco * vanco[i] + b_abx_vancopo * vancopo[i] + b_abx_metro * metro[i] + b_abx_cefaz * cefaz[i] + b_abx_dapto * dapto[i] + b_abx_linez * linez[i] + b_abx_mero * mero[i] + b_abx_piptaz * piptaz[i] + b_abx_cefep * cefep[i] + alpha;
    }
