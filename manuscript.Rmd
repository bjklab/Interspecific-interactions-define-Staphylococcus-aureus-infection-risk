---
#title: "Interspecific interactions define respiratory _Staphylococcus aureus_ infection risk during mechanical ventilation"  
...
## Title  
Interspecific interactions define respiratory _Staphylococcus aureus_ infection risk during mechanical ventilation  
  
## Running Title  
Respiratory _Staph aureus_ interactions  
...
Intranasal colonization with _Staphylococcus aureus_ ( _Sa_ ) is a marker for increased risk of invasive _Sa_ infection. Nasal bacteria have been shown to interact with _Sa_ and impact risk of colonization, but their impact on invasive disease is unknown. We sought to define the relationship between intranasal and respiratory tract microbiota among critically ill subjects dependent on mechanical ventilation and identify features that impact risk of _Sa_ lower respiratory tract infection (LRTI). We enrolled two prospective cohorts (pilot n = `r micu_cohort %>% filter(grepl("pilot",subject_id)) %>% pull(subject_id) %>% unique() %>% length()`; validation n = `r micu_cohort %>% filter(grepl("validation",subject_id)) %>% pull(subject_id) %>% unique() %>% length()`), collected longitudinal anterior nares (AN) and endotracheal aspirate (ET) specimens, and analyzed bacterial community composition by 16S ribosomal RNA gene sequencing. Three intranasal _Sa_ amplicon sequence variants (ASVs) were associated with increased _Sa_ abundance in both AN and ET sites, as well as increased risk for _Sa_ LRTI. Of 597 non- _Sa_ intranasal ASVs, 11 were identified as protective against _Sa_ LRTI, adjusting for abundance of _Sa_ ASVs and prior antibiotic exposure. 8 of 11 (72.7%) protective ASVs were Corynebacterium species. During mechanical ventilation, the risk of _Sa_ ventilator-associated LRTI increases with abundance of key _Sa_ ASVs, but interspecific interactions largely driven by _Corynebacterium_ species decrease the risk of invasive disease posed by _Sa_ nasal colonization.  
